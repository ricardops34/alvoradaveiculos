...
### ⚖️ Comparador de Anúncios (22/07)
- [x] **Seleção de veículos para comparar**: botão "⚖" em cada card da listagem (`LandingComponent`), ao lado do favorito (mesmo padrão de `event.preventDefault()`/`stopPropagation()` pra não disparar a navegação do card). Seleção é anônima (não exige login de Cliente, ao contrário dos favoritos) e persistida em `localStorage` via novos métodos no `LojaService` (`getCompararIds`/`alternarComparar`/`removerComparar`/`limparComparar`), limitada a 4 veículos por vez. Barra flutuante no rodapé da tela mostra a contagem selecionada, com "Limpar" e "Comparar agora" (habilitado a partir de 2 veículos).
- [x] **Tela `/comparador`** (`ComparadorComponent`, nova, seguindo o mesmo padrão visual/guarda `lojaGuard` da Landing e do Detalhe): lê os ids da query string (`?ids=1,3,4`), busca cada veículo via `GET /api/loja/veiculos/:id` (não foi criado endpoint novo — a própria API pública já retorna tudo que a comparação precisa por veículo, incluindo a ficha técnica do modelo) e monta uma tabela lado a lado: foto, marca/modelo/versão, Preço/Ano/Cor/Quilometragem/Tipo, seção de Ficha Técnica (só exibe as linhas onde ao menos um veículo tem valor) e seção de Opcionais (união de todos os opcionais dos veículos comparados, com check/traço por veículo). Botão "✕" em cada coluna remove o veículo da comparação (atualiza a URL); ao remover o último, volta pro estoque.
- [x] **Validado de ponta a ponta** contra Postgres + API + frontend reais (stack de dev subida localmente, `npm run seed:teste` + ajustes manuais de `publicado`/`opcionais`/uma ficha técnica de teste): confirmado por screenshot que a tela `/comparador` monta corretamente a tabela com 3 veículos reais (preço, ano, cor, km, ficha técnica só no veículo que tinha e "—" nos outros, opcionais com check/traço corretos), que o estado vazio (`/comparador` sem `ids`) mostra a mensagem de "nenhum veículo selecionado", e que ids inválidos/não publicados são filtrados sem quebrar a tela.
  - ⚠️ **Não testado nesta sessão**: os cliques reais nos botões "⚖"/"✕" e a barra flutuante dentro de um navegador interativo (o ambiente não tinha `chromium-cli`/Playwright disponível, só screenshots estáticos via Chrome headless por URL). A lógica é a mesma já validada em produção para o toggle de favoritos (mesmo padrão de `event.stopPropagation()` + `Set` local sincronizado com uma fonte de verdade), mas vale um teste manual num navegador de verdade antes de dar como 100% concluído.
